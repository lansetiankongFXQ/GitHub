...
def parse_roll(response):
    ...
    try:
        link = LinkExtractor(restrict_xpaths='//ul[@class="f14 lh30 f12_5a5a5a left"]/li')
...
def parse_article_publish_time(tree_node):
    # 央广网
    try:
        publish_time_node = tree_node.xpath('//div[@class="subject"]/div[@class="source"]/span[1]/text()')
...
def parse_article_title(tree_node):
    ...
    try:
        title = tree_node.xpath('//div[@class="article"]/div[@class="subject"]/h2/text()')
        if title:
...
def parse_article_original_source(tree_node):
    ...
    try:
        original_source = tree_node.xpath('//div[@class="subject"]/div[@class="source"]/span[2]/text()')
        if original_source and original_source[0]:
...
def parse_page_subject(tree_node):
    ...
    try:
        page_subject = tree_node.xpath('//p[@class="daoHang"]/a/text()')
        if page_subject:
            page_subject_str = ''
